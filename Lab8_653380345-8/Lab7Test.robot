...
${CHROME PATH}     ${EXECDIR}${/}Test${/}chrome-win64${/}chrome-win64${/}chrome.exe
${CHROME DRIVER}   ${EXECDIR}${/}Test${/}chrome-win64${/}chrome-win64${/}chromedriver.exe

*** Keywords ***
Open Browser To Form Page
    [Documentation]    Opens the browser and navigates to the form page.
    ${options}=    Evaluate    sys.modules['selenium.webdriver'].ChromeOptions()    sys, selenium.webdriver
    ${options.binary_location}    Set Variable    ${CHROME PATH}
    ${service}=    Evaluate    sys.modules['selenium.webdriver.chrome.service'].Service(executable_path=r'${CHROME DRIVER}')    sys, selenium.webdriver.chrome.service
